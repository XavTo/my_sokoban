...
char **tab_swaps(char **str, int *player, int sens, obj_t *obj)
{
    ...
    if (sens == 2) {
        if (str[b + 1][a] == 'O' || str[b + 1][a] == '#') {
            if (str[b + 1][a] == 'O') str[b + 1][a] = 'P', str[b][a] = ' ';
        }
        else if (str[b + 1][a] == 'X') my_change_x(str, player, sens);
        else temp = str[b][a], str[b][a] = str[b + 1][a], str[b + 1][a] = temp;
    }
    if (sens == 3) {
        if (str[b - 1][a] == 'O' || str[b - 1][a] == '#') {
            if (str[b - 1][a] == 'O') str[b - 1][a] = 'P', str[b][a] = ' ';
...
char **tab_swap(char **str, int *player, int sens, obj_t *obj)
{
    ...
    if (sens == 0) {
        if (str[b][a + 1] == 'O' || str[b][a + 1] == '#') {
            if (str[b][a + 1] == 'O') str[b][a + 1] = 'P', str[b][a] = ' ';
        }
        else if (str[b][a + 1] == 'X') my_change_x(str, player, sens);
        else temp = str[b][a], str[b][a] = str[b][a + 1], str[b][a + 1] = temp;
    }
    if (sens == 1) {
        if (str[b][a - 1] == 'O' || str[b][a - 1] == '#') {
            if (str[b][a - 1] == 'O') str[b][a - 1] = 'P', str[b][a] = ' ';
